...
using Automation.DynamicForms.Features.Structs.CreateStruct;
using Automation.DynamicForms.Features.Structs.DeleteStruct;
using Automation.DynamicForms.Features.Structs.GetProjectStructs;
using Automation.DynamicForms.Features.Structs.GetStructById;
using Automation.DynamicForms.Features.Structs.UpdateStruct;
...
public class StructCrudHandlerTests
{
    ...
    {
        using var db = CreateInMemoryDbContext(nameof(CreateStruct_ShouldComputeDependenciesAndSucceed));
        ...
        var childResult = await createChildHandler.HandleAsync(
            new CreateStructCommand(projectId, "BaseTransform", JsonDocument.Parse("""[{"name":"x","type":"number"}]""")),
    ...
    {
        using var db = CreateInMemoryDbContext(nameof(CreateStruct_ShouldFail_WhenDuplicateNameInSameProject));
        var projectId = Guid.NewGuid();
        var handler = new CreateStructHandler(db);

        var res1 = await handler.HandleAsync(new CreateStructCommand(projectId, "UniqueStruct"), CancellationToken.None);
        res1.IsSuccess.Should().BeTrue();

        var res2 = await handler.HandleAsync(new CreateStructCommand(projectId, "UniqueStruct"), CancellationToken.None);
    ...
    {
        using var db = CreateInMemoryDbContext(nameof(UpdateStruct_ShouldFailFast_WhenCircularDependencyDetected));
        var projectId = Guid.NewGuid();
        var createHandler = new CreateStructHandler(db);

        // Tạo Struct A
        var resA = await createHandler.HandleAsync(new CreateStructCommand(projectId, "StructA"), CancellationToken.None);
        ...
        """);
        var resB = await createHandler.HandleAsync(new CreateStructCommand(projectId, "StructB", fieldsB), CancellationToken.None);
    ...
    {
        using var db = CreateInMemoryDbContext(nameof(DeleteStruct_ShouldBlockDeletion_WhenReferencedByAnotherStruct));
        ...
        // Tạo Struct Con
        var resChild = await createHandler.HandleAsync(new CreateStructCommand(projectId, "ChildStruct"), CancellationToken.None);
        ...
        """);
        var resParent = await createHandler.HandleAsync(new CreateStructCommand(projectId, "ParentStruct", parentFields), CancellationToken.None);

        // Thử xóa Struct Con
        var deleteHandler = new DeleteStructHandler(db, api);
        var deleteResult = await deleteHandler.HandleAsync(new DeleteStructCommand(projectId, childId), CancellationToken.None);

        deleteResult.IsFailed.Should().BeTrue();
        deleteResult.Errors[0].Message.Should().Contain("Cannot delete Struct 'ChildStruct' because it is referenced by");
        deleteResult.Errors[0].Message.Should().Contain("ParentStruct");

        // Xóa Struct Cha trước
        var deleteParentResult = await deleteHandler.HandleAsync(new DeleteStructCommand(projectId, resParent.Value.Id), CancellationToken.None);
        deleteParentResult.IsSuccess.Should().BeTrue();

        // Bây giờ xóa Struct Con phải thành công
        var deleteChildAfterParentResult = await deleteHandler.HandleAsync(new DeleteStructCommand(projectId, childId), CancellationToken.None);
    ...
    {
        using var db = CreateInMemoryDbContext(nameof(GetProjectStructs_ShouldFilterAndReturnSummaryDtos));
        var projectId = Guid.NewGuid();
        var createHandler = new CreateStructHandler(db);

        await createHandler.HandleAsync(new CreateStructCommand(projectId, "AlphaStruct"), CancellationToken.None);
        await createHandler.HandleAsync(new CreateStructCommand(projectId, "BetaStruct"), CancellationToken.None);

        var getHandler = new GetProjectStructsHandler(db);
        var allList = await getHandler.HandleAsync(new GetProjectStructsQuery(projectId), CancellationToken.None);

        allList.IsSuccess.Should().BeTrue();
        allList.Value.Should().HaveCount(2);

        var filteredList = await getHandler.HandleAsync(new GetProjectStructsQuery(projectId, "alpha"), CancellationToken.None);
